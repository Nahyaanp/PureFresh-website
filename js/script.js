...
(function($) {
	var settings;
	$.fn.Purefresh = function(actionOrSettings, parameter) {
		if (typeof actionOrSettings === 'object' || actionOrSettings === undefined) {
			// Default settings:
			settings = $.extend({
				// To use a headline tag other than h3, adjust or overwrite Purefresh.css as well
				headline: 'h3',
		...
		if (actionOrSettings == "open") {
			...
			$(settings.headline).Purefresh('close');
		} else if (actionOrSettings == "forceCloseAll") {
			// forceCloseAll ignores collapsible setting
			$(settings.headline).Purefresh('forceClose');
		}
		...
		$(settings.headline, this).click(function() {
			if ($(this).hasClass('active')) {
				$(this).Purefresh('close');
			} else {
				$(this).Purefresh('open', settings.scroll);
			}
		});
	};
}(jQuery));
